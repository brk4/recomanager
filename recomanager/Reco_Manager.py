    ...
    if datasetFileName is None:
        ...
        if datasetIsValid==True:        
            ...
            if os.path.isdir("tiff"):
                logfileParameters.datasetType="tiff"
                ...
                IJ.showMessage("The data formats other than \"tiff\" or \"h5\" are not supported yet!")
            ...
            if logfileParameters.datasetType!="":
                if world=="offline":
                    ...
                    if linelist[1]=="sls":
                        ...
                        if logfileParameters.datasetType=="tiff":
                            fakeTifDir=logfileParameters.datasetOut+"/tiff"
                            ...
                            if os.path.isdir(logfileParameters.datasetOut):
                                if os.path.isdir(fakeTifDir):
                                    if os.path.isfile(fakeLogfile):
                                        ...
                                        command="cp "+logfileParameters.dataset + "/tiff/" + logfileParameters.samplename + ".log " + fakeTifDir
                                        os.system(command)
                                else:
                                    os.makedirs(logfileParameters.datasetOut+"/tiff")
                                    command="cp "+logfileParameters.dataset + "/tiff/" + logfileParameters.samplename + ".log " + fakeTifDir
                                    os.system(command)
                            else:
                                os.makedirs(logfileParameters.datasetOut+"/tiff")
                ...
                if logfileParameters.beamlineStorage==1:
                    ...
                    if logfileParameters.datasetType=="tiff":
                        logfileParameters.logfile = logfileParameters.dataset + "/tiff/" + logfileParameters.samplename + ".log"
                    else:
...
def reconstruct(event):
    ...
    if event.getSource() == oneSliceButton:
        ...
        if recoParameters.algorithm==0:
         ...
         #   print("The absorption sinograms for " + logfileParameters.samplename + " do not exist yet and need to be (or are currently being) computed." )
        #elif sinogramCalculation.phaseSinogramFound==0 and recoParameters.algorithm==1:
            #print("The Paganin sinograms for " + logfileParameters.samplename + " do not exist yet and need to be (or are currently being) computed." )
        #else:
    ...
    def actionPerformed(e,e1):
        ...
        if flatFieldBox.isSelected() == True:
            ...
            if logfileParameters.datasetType=="tiff":
                ...
                projection = logfileParameters.dataset +  "/tiff/" + logfileParameters.samplename + str(int(chosenSlice)+1).zfill(projectionNumberOfDigits) + ".tiff"
...
class flatFieldSelection(event.ItemListener):
    def itemStateChanged(e,e1):
        ...
        if flatFieldBox.isSelected() == True:
            ...
            chosenSlice = projectionSelection.getSelectedIndex()
            if logfileParameters.datasetType=="tiff":
                projection = logfileParameters.dataset +  "/tiff/" + logfileParameters.samplename + str(int(chosenSlice)+1).zfill(4) + ".tiff"
                proj=IJ.openImage(projection)
                darkProjection = logfileParameters.dataset +  "/tiff/" + logfileParameters.samplename + str(1).zfill(4) + ".tiff"
                darkImg=IJ.openImage(darkProjection)
                flatProjection = logfileParameters.dataset +  "/tiff/" + logfileParameters.samplename + str(int(logfileParameters.ndrk)+1).zfill(4) + ".tiff"
            ...
            title = "Flat-field corrected " + logfileParameters.samplename + str(int(chosenSlice)+1).zfill(4) + ".tiff"
            img.setTitle(title)            
            img.show()
            win=img.getWindow()
            canvas = win.getCanvas()
